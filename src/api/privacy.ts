...
privacyRouter.get(
  '/anonymity-sets/:protocol/history',
  asyncHandler(async (req: Request, res: Response) => {
    try {
      const protocol = req.params.protocol.toUpperCase();
      const days = Math.min(365, Math.max(1, Number(req.query.days) || 30));
        ...
        .map(([address, stats]) => ({
          ...
          avgRiskScore:
            stats.risks.length > 0
              ? stats.risks.reduce((a, b) => a + b, 0) / stats.risks.length
              : 0,
          protocolCount: stats.protocols.size,
        }))
...
privacyRouter.get(
  '/compliance/report/:address/export',
  asyncHandler(async (req: Request, res: Response) => {
    try {
...
privacyRouter.get(
  '/de-anonymization/timing/:address',
  asyncHandler(async (req: Request, res: Response) => {
    try {
...
privacyRouter.get(
  '/de-anonymization/amount/:address',
  asyncHandler(async (req: Request, res: Response) => {
    try {
...
privacyRouter.get(
  '/de-anonymization/taint/:address',
  asyncHandler(async (req: Request, res: Response) => {
    try {
